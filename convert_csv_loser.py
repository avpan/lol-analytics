...
def main(matchNum):
	...
	#remove winners of match and keep losers
	losers=[]
	for item in data["participants"]:
		...
		if not item['stats']['winner']:
			...
			if lane == 'BOTTOM':
				...
				if role == 'DUO_CARRY':
					...
					lane = 'SUPPORT'
			losers.append([item['participantId'],lane,kills,deaths,assists,rank,gold,cs])
		if item['stats']['winner']:
	...
	datafile = open('./data/csv_loser/data_match%d.csv'%matchNum,'w')
	for i in range(0,T):
	...
	T = len(time)
	df_top = open('./data/timeline_loser/top/data_match%d.csv'%matchNum,'w')
	df_mid = open('./data/timeline_loser/mid/data_match%d.csv'%matchNum,'w')
	df_jung = open('./data/timeline_loser/jungle/data_match%d.csv'%matchNum,'w')
	df_adc = open('./data/timeline_loser/adc/data_match%d.csv'%matchNum,'w')
	df_sup = open('./data/timeline_loser/support/data_match%d.csv'%matchNum,'w')
	for i in range(0,T):
	...
	endstats_df = open('./data/endstats_loser/data_match%d.csv'%matchNum,'w')
