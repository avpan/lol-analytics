...
def findAvg():
		plotdata = open('./data/FINAL/FINAL_loser.csv','w')
		f = open('./data/FINAL/final_data_losers.csv','r')
		data= f.read()
...
def main(num,cur_time):
		filepath = './data/csvdata_loser/data_match%d.csv' % num
		finalpath = './data/FINAL/final_data_winners.csv' #path for winners
		finalpath = './data/FINAL/final_data_losers.csv' #path for losers
		f=open(filepath,'r')
...
if __name__ == "__main__":
	...
	for i in range(1,FILENUM):
		filepath = './data/csvdata_loser/data_match%d.csv' % i
